...
public class UserAccount {
    ...
    public String checkDuplicateField(String username, String email, String phone, int excludeUserId) {
        String sql = "SELECT username, email, phone_number FROM user_account WHERE (LOWER(username) = LOWER(?) OR LOWER(email) = LOWER(?) OR phone_number = ?) AND user_id != ? LIMIT 1";
        try (Connection c = DBUtils.getConnection(); PreparedStatement ps = c.prepareStatement(sql)) {
            ...
            try (ResultSet rs = ps.executeQuery()) {
                if (rs.next()) {
                    // 拿到冲突的数据后，对比一下到底是谁撞了
                    if (username.equalsIgnoreCase(rs.getString("username"))) return "Username";
                    if (email.equalsIgnoreCase(rs.getString("email"))) return "Email";
                    if (phone.equals(rs.getString("phone_number"))) return "Phone number";
                }
            }
